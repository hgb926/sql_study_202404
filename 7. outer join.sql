...
SELECT -- outer join은 다 보여주는 대신 매칭안되는 부분은 null로 처리.
    *
FROM tb_emp E
LEFT JOIN tb_dept D -- outer 키워드 생략 가능
ON E.dept_cd = D.dept_cd
;
...
-- 부서에 소속된 사원이 41명
-- 사원이 존재하는 부서 14개
-- INNER JOIN의 결과 사원정보 조회 41건
-- 왜? INNER JOIN은 부서가 있는 사람들만 매칭해서 가져온다
-- LEFT JOIN : INNER 41 + 부서가 없는 사원 5 = 46건
-- RIGHT JOIN : INNER 41 + 사원이 없는 부서 2 = 43건
-- FULL JOIN : INNER 41 + 5 + 2 = 48건
-- OUTER JOIN의 결과는 항상 INNER JOIN의 결과를 가지고 있다.
