...
SELECT 
    emp_no, -- 사번
    TO_CHAR(SUM(pay_amt), 'L999,999,999'), "각 사원들의 월급 총합"
    TO_CHAR(ROUND(AVG(pay_amt),2),'L999,999,999'), "각 사원들의 월급 평균"
    TO_CHAR(MAX(pay_amt), 'L999,999,999'), "각 사원들의 최대 월급"
    COUNT(pay_de) "각 사원들이 월급 얼마나 받았는지"
FROM tb_sal_his
GROUP BY emp_no
ORDER BY emp_no
;
...
SELECT 
    emp_no, -- 사번
    TO_CHAR(SUM(pay_amt), 'L999,999,999') "각사원별 총급여액",
...
;
-- ORDER BY 두개 이상이면 첫번째 정렬 중 겹치면 두번째로 정렬 ...
-- dept_cd ASC(숨어져있음), 부서코드가 동일하면 emp_nm으로 DESC 하라
